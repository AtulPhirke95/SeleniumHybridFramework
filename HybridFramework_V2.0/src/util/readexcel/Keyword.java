...
public class Keyword {
	...
	public String performAction(TestStepClass tsObj, WebDriver wdObj)
		...
		case ("deselectbyindex"):
			return actionDeSelectByIndex(getWebElementObject(), tsObj.getData());

		case ("selectbyvisibletext"):
			return actionSelectByVisibleText(getWebElementObject(), tsObj.getData());

		case ("deselectByVisibleText"):
			return actionDeselectSelectByVisibleText(getWebElementObject(), tsObj.getData());
		
		case ("selectbyvalue"):
			return actionSelectByValue(getWebElementObject(), tsObj.getData());
		
		case ("deselectByValue"):
			return actionDeselectByValue(getWebElementObject(), tsObj.getData());
	...
	public String actionSelectByIndex(WebElement webEleObj, String data) {
		...
		try {
			int indexOfstartOfDot = data.indexOf(".");
			int tempData =Integer.parseInt(data.substring(0, indexOfstartOfDot));
			sel.selectByIndex(tempData);
	...
	public String actionDeSelectByIndex(WebElement webEleObj, String data) {
		Select sel = new Select(webEleObj);
		try {
			int indexOfstartOfDot = data.indexOf(".");
			int tempData =Integer.parseInt(data.substring(0, indexOfstartOfDot));
			sel.deselectByIndex(tempData);
	...
	public String actionDeselectByValue(WebElement webEleObj, String data) {
		Select sel = new Select(webEleObj);
		try {
			sel.deselectByValue(data);
		} catch (Exception e) {
			return "Failure : " + e.getMessage();
		}
		return "Success";
	...
	public String actionDeselectSelectByVisibleText(WebElement webEleObj, String data) {
		Select sel = new Select(webEleObj);
		try {
			sel.deselectByVisibleText(data);
		} catch (Exception e) {
			return "Failure : " + e.getMessage();
		}
		return "Success";
	...
	public String actionSendKeys(WebElement webEleObj, String data) {
		if(data.startsWith("(")){
			tempColumnValue = objFetchDataFromExcel.fetchingData(data);
			webEleObj.sendKeys(tempColumnValue);
		}else{
